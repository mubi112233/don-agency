...
export default function ResourcesSection({ language }: ResourcesSectionProps) {
    ...
    <section className="px-6 md:px-8 py-20 md:py-32 max-w-[1400px] mx-auto border-t border-border">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        {t.cards.map((card, i) => (
          <div
            key={i}
            className="bg-card border border-border p-8 rounded-sm hover:border-primary/40 transition-all duration-300 group cursor-pointer"
          >
            <div className="text-sm text-primary font-medium mb-3">{card.type}</div>
            ...
            <div className="flex items-center text-primary group-hover:gap-2 transition-all duration-200">
              <span className="text-sm font-medium">{card.readMore}</span>
